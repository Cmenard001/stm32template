...
function(generate_cubemx_code BSP_NAME)
    ...
    if(NOT EXISTS "${BSP_BUILD_DIR}/Core" OR NEED_REGENERATION)
        ...
        file(WRITE "${BUILD_CUBEMX_SCRIPT}" "project generate\nexit\n")
        ...
        message(STATUS "Execute command: ${CUBEMX_COMMAND} \"${BUILD_IOC_FILE}\" -q \"${BUILD_CUBEMX_SCRIPT}\"")

        # Execute CubeMX
        execute_process(
            COMMAND ${CUBEMX_COMMAND} "${BUILD_IOC_FILE}" -q "${BUILD_CUBEMX_SCRIPT}"
...
function(include_generated_cubemx TARGET_NAME BSP_NAME)
    set(BSP_BUILD_DIR "${CMAKE_BINARY_DIR}/bsp/${BSP_NAME}")
    ...
    # Collect Core sources (include main.c which is now bsp_init())
    file(GLOB CORE_SOURCES "${BSP_BUILD_DIR}/Core/Src/*.c")

    # Find startup file (check both root and Core/Startup/)
    file(GLOB STARTUP_FILES "${BSP_BUILD_DIR}/startup_*.s" "${BSP_BUILD_DIR}/Core/Startup/startup_*.s")
